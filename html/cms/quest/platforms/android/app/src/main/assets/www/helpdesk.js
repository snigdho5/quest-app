 ...
 */
var helpdeskserver = 'https://access.cesc.co.in/corpdesk_api/';
    ...
    try {
        ...
        $.post(url, {
          ...
          //  });
                    ...
                    if((myObj.data[0].status == '3') && myObj.data[0].feedback == null)
                    {
                        innerHtml = innerHtml + ' <input name="ticket_id" type="hidden" value="1"><fieldset><legend>Your Feedback</legend>';
                        innerHtml = innerHtml + '<div class="form-group"><div class="col-lg-12">';
                      //  innerHtml = innerHtml + '<textarea class="form-control" rows="3" name="feedback" id="feedback" cols="50">'+myObjdata['feedback']+'</textarea></div></div></fieldset></form></div></div>';
        ...
        $.post(url, {
           ...
           // $.each(myObj, function (key, value) {
               // myObjdata = myObj[key];
                ...
                innerHtml = innerHtml + '<p><strong>Intecom</strong>: ' + '<span style="color: #22CC8B">' + myObj.data[0].intercom + '</span></p>';
                innerHtml = innerHtml + '</div>';
                ...
                innerHtml = innerHtml + '<div class="form-group"><div class="col-lg-12">';
                innerHtml = innerHtml + '<textarea class="form-control" rows="3" name="reply" id="reply" cols="50"></textarea></div></div><div class="text-right col-md-12"><input class="btn btn-primary" type="button" onClick="insertReply(' + myObj.data[0].docket_no + ')" value="Submit"></div></fieldset></form></div></div>';
                //innerHtml = innerHtml + '<div id="feedback_msg"></div>';
