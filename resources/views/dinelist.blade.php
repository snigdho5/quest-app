            ...
            <div class="row">
                <div class="formgroup col-8 d-block d-md-none d-lg-none pr-0">
                    <div class="input-group">
                        <input type="text" class="form-control rounded-0" id="storeSearch" placeholder="Type dine name or food type" onkeyup="$('.storeSearch').val($(this).val())">
        ...
        <div class="allcategory">
            <div class="row">
                    ...
                    <div class="position-sticky" style=" top: 25px;">
                        <form action="" id="searchForm">
                            <div class="input-group mb-4 storesearch">
                                <input name="search" type="text" class="form-control rounded-0 storeSearch" id="storeSearch" placeholder="Type name of diner or food type" value="{{ request()->search }}">
                                <div class="input-group-append">
                                    <button class="btn btn-secondary searchbutton rounded-0" ><i class="fa fa-search" aria-hidden="true"></i></button>
                                </div>
                            </div>
                            <select name="type" class="form-control filterSearch mb-4 rounded-0" onchange="this.form.submit()">
                            ...
                            <select name="floor" class="form-control filterSearch mb-4 rounded-0" onchange="this.form.submit()">
                                ...
                                <option {{ request()->floor=="Fourth Floor"?'selected':'' }}>Fourth Floor</option>
                                <option {{ request()->floor=="Fifth Floor"?'selected':'' }}>Fifth Floor</option>
                                <option {{ request()->floor=="Sixth Floor"?'selected':'' }}>Sixth Floor</option>
                            </select>
                ...
                <div class="col-md-8" id="dealsbox">
                    <div class="row">
                        @if($dineData->count()==0)
                        <div class="col-sm-12"><h4 class="mt-5 mb-5 text-center text-muted">Sorry! We couldn't find any dine.<br><small>Please try again with different preferences.</small></h4>
...
<script type="text/javascript">
    ...
    var working = false;
    var total = {{ $dineCount }};
    var current = {{ $dineData->count() }};
    function loadMore(){
        if((total > current) && !working){
            working = true;
            $.ajax({
                url: 'ajax/loadDine{!! $_SERVER['QUERY_STRING']?'?'.$_SERVER['QUERY_STRING']:''!!}',
                type: 'POST',
                data: {count: current},
