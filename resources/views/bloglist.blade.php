        ...
        <div class="row b_cont">
            ...
            <div class="col-lg-4 col-md-6 col-12 mb-4">
                ...
                <div class="card rounded-0 border-0">
                    <div>
                        <a href="blogs/{{ $value->slug }}"><img class="img-fluid w-100" src="{{ asset('storage/blog/thumb/'.$value->sq_image) }}"></a>
                    </div>
                    <div class="page_header">
                        <h6 class="text-uppercase mb-0 text-white">{{ date('dS M, y',strtotime($value->post_time)) }}</h6>
                        <ul class="mb-0">
                            <li><a href="https://www.facebook.com/sharer/sharer.php?u={{ url('blogs/' . $value->slug) }}" target="_blank"><i class="fa fa-facebook" aria-hidden="true"></i></a></li>
                            <li><a href="https://twitter.com/intent/tweet?url={{ url('blogs/' . $value->slug) }}" target="_blank"><i class="fa fa-twitter" aria-hidden="true"></i></a></li>
                            <li><a href="http://pinterest.com/pin/create/button/?url={{ url('blogs/' . $value->slug) }}&media={{ asset('storage/blog/thumb/'.$value->sq_image) }}" target="_blank"><i class="fa fa-pinterest-p" aria-hidden="true"></i></a></li>
                        </ul>
                    </div>
                    <div class="card-body text-center">
                        <h5 class="card-title"><a href="blogs/{{ $value->slug }}">{{ $value->title }}</a></h5>
                        <p class="card-text">{{ str_limit(strip_tags($value->content),70) }}</p>
                        <a href="blogs/{{ $value->slug }}" class="text-uppercase r_more">Read More</a>
                    </div>
                </div>
