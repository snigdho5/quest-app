  ...
  <section class="content">
    ...
    <div class="box">
      <div class="box-header">
        ...
        <a class="btn btn-primary btn-sm pull-right" href="admin/store/{{ $store_id }}/deal/create/">Add New</a></h3>
        @endif
      </div>
      <div class="box-body">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $value->active=='0'?'No':'Yes' }}
                ...
                <a href="admin/store/{{ $store_id }}/deal/status/{{ $value->id }}/{{ $value->active=='0'?'1':'0' }}"><i class="fa fa-refresh fa-fw"></i></a>
                @endif
              </td>
              <td class="text-right">
                @if(in_array('edit',$access_type))
                <a title="Edit" class="btn btn-success btn-xs" href="admin/store/{{ $store_id }}/deal/edit/{{ $value->id }}"><i class="fa fa-pencil fa-fw"></i></a>
                @endif

                @if(in_array('delete',$access_type))
                <a title="Delete" class="btn btn-danger btn-xs" onclick="return confirm('Once deleted it cannot be restored. Are you sure you want to delete this?')" href="admin/store/{{ $store_id }}/deal/delete/{{ $value->id }}"><i class="fa fa-close fa-fw"></i></a>
                @endif
              </td>
            </tr>
            @endforeach
