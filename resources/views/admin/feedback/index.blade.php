...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Feedback/Complain</li>
    </ol>
  </section>
  <!-- Main content -->
  <section class="content">
    ...
    <div class="box">
      <div class="box-header">
        <h3 class="box-title" style="display:block">Feedback/Complain List</h3>
      </div>
      <div class="box-body">
        <table id="listtable" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td> {{($value->user_id!=0)?get_anydata('user',$value->user_id,'name'):$value->name }}</td>
              <td> {{($value->user_id!=0)?get_anydata('user',$value->user_id,'email'):$value->email }}</td>
              <td> {{($value->user_id!=0)?get_anydata('user',$value->user_id,'phone'):$value->mobile }}</td>
              <td> {{ get_anydata('store',$value->store_id,'name') }}</td>
              <td>{{$value->type}}</td>
              <td> {{ ($value->feedback!='')?str_limit(strip_tags($value->feedback),70):'--' }}</td>
              

              <td class="text-right">
                @if(in_array('edit',$access_type))
                <a title="View" class="btn btn-success btn-xs" href="admin/feedback/view/{{ $value->id }}"><i class="fa fa-eye fa-fw"></i></a>
                @endif
...
<script>
  $(document).ready(function() {
   $('#listtable').DataTable({
      ...
      buttons: [
        {
          'filename':'Feedback/Complain_Transaction_List_{{date('Y-m-d')}}',
          'extend':'excelHtml5',
          'exportOptions':{
