...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Referral User</li>
    </ol>
  </section>
  <!-- Main content -->
  <section class="content">
    ...
    <div class="box">
      <div class="box-header">
        <h3 class="box-title" style="display:block">Referral User List</h3>
      </div>
      <div class="box-body">
        <table id="listtable" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td> {{ get_anydata('user',$value->new_user_id,'name') }}</td>
              <td>{{ $value->created_at->format('M d, Y - h:i a') }}</td>
              {{-- <td>{{ $value->status=='0'?'No':'Yes' }} <a href="admin/referral/status/{{ $value->id }}/{{ $value->status=='0'?'1':'0' }}"><i class="fa fa-refresh fa-fw"></i></a></td> --}}
              <td class="text-right">
                @if(in_array('edit',$access_type))
                <a title="View" class="btn btn-success btn-xs" href="admin/referral/view/{{ $value->id }}"><i class="fa fa-eye fa-fw"></i></a>
                @endif

                {{-- @if(in_array('delete',$access_type))
                <a title="Delete" class="btn btn-danger btn-xs" onclick="return confirm('Once deleted it cannot be restored. Are you sure you ant to delete this?')" href="admin/referral/delete/{{ $value->id }}"><i class="fa fa-close fa-fw"></i></a>
                @endif --}}
              </td>
            </tr>
            @endforeach
...
<script>
  $(document).ready(function() {
    $('#listtable').DataTable({
      ...
      buttons: [
        {
          'filename':'Referral_User_{{date('Y-m-d')}}',
          'extend':'excelHtml5',
          'exportOptions':{
