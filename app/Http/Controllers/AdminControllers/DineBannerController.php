...
class DineBannerController extends Controller
{
    ...
    public function index($store_id)
    {
        ...
        return view('admin.dine_banner.index',compact('data','access_type', 'store_id'));
    }
    ...
    public function create($store_id)
    {
        ...
        return view('admin.dine_banner.create',compact('store_id'));
    }
    ...
    public function store($store_id, Request $request)
    {
        ...
        session()->flash('success','Dine Banner added successfully.');
        return redirect()->route('admin::dineBanner',$store_id);
    }

    public function edit($store_id, $id, StoreBanner $dineBanner)
    {
        abort_unless(in_array('edit',$this->access_type), 403);
        $data = $dineBanner->find($id);
        return view('admin.dine_banner.edit',compact('data','store_id'));
    }

    public function update($store_id, $id, Request $request, StoreBanner $dineBanner)
    {
        abort_unless(in_array('edit',$this->access_type), 403);
        // validation
        $this->validate(request(),[
        ...
        if (request()->hasFile('imagefile')) {
            ...
            // delete old file
            $old_file = Storage::disk('public')->path($this->uploadPath.$dineBanner->image);
            if(is_file($old_file)) Storage::disk('public')->delete($this->uploadPath.$dineBanner->image);
            $old_file = Storage::disk('public')->path($this->uploadPath_thumb.$dineBanner->image);
            if(is_file($old_file)) Storage::disk('public')->delete($this->uploadPath_thumb.$dineBanner->image);
        ...
        if (request()->hasFile('aimagefile')) {
            ...
            // delete old file
            $old_file = Storage::disk('public')->path($this->uploadPath_app.$dineBanner->app_image);
            if(is_file($old_file)) Storage::disk('public')->delete($this->uploadPath_app.$dineBanner->app_image);

            request()->merge(['app_image' => $aimagename]);
        }

        // update database
        $dineBanner->update(request()->all());
        $dineBanner->save();
        ...
        // set message and redirect
        session()->flash('success','Dine Banner updated successfully.');
        return redirect()->route('admin::dineBanner',$store_id);
    }

    public function destroy($store_id, $id, StoreBanner $dineBanner)
    {
        ...
        // reset priority
        StoreBanner::resetOrder($dineBanner->priority,$dineBanner->store_id);

        // delete data
        $dineBanner->delete();

        // set message and redirect
        session()->flash('success','Dine Banner deleted successfully.');
        return redirect()->route('admin::dineBanner',$store_id);
    }
    ...
    public function change_status($store_id, $id,$type)
    {
        ...
        // set message and redirect
        session()->flash('success','Dine Banner status changed successfully.');
        return redirect()->route('admin::dineBanner',$store_id);
    }
